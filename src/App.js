...
class App extends React.Component {
  constructor(props) {
    ...
    this.state = {
      colorMode: "blue",
      visitCount: localStorage.getItem("page_view") ?? 1,
    };

    localStorage.setItem("page_view", ++this.state.visitCount);
  }
  ...
  render() {
    return (
      ...
      >
        <RandomFact />
        <Resume
          colorTheme={colorThemes[this.state.colorMode]}
        ...
        <div className="styleContainer">
          <div className="styleButtonGroup">
            ...
            <button
              className="button-clear styleButton"
              onClick={() =>
                this.props.setIsNinetiesMode(!this.props.isNinetiesMode)
              }
            >
              <span className="styleButtonIcon">
        ...
        <div className="downloadLink">
          <a href={require("./Andrew_Torr_Resume.pdf")} download>
            Download Resume
          </a>
        </div>
        {this.props.isNinetiesMode && (
          <div className="visitorCounter">
            ...
            <div className="visitorCounter-number">{this.state.visitCount}</div>
          </div>
        )}
      </div>
    );
